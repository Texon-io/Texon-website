...
function Services() {
    return (
        <div className={`flex justify-between items-center h-screen  px-[105px]`}>
            <div className={`services-text h-full flex flex-col justify-center`}>
                <DashedHeading  heading={`Our Services.`} para={`Focused, fast, and built to make a difference.`}/>
            </div>
                ...
                >
                    {cardsData.map(card => (
                        <Card key={card.service} className={`shadow-lg relative rounded-xl p-6 flex flex-col gap-3 border border-gray-200 dark:border-gray-700 hover:scale-105 transition-transform duration-300 flex-center`}>
                            ...
                            <img className={`w-full h-full inset-0 opacity-45 saturate-50 -z-10 absolute`} src={cardBg}/>
