...
function Navbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-main-black h-16 px-6 md:px-12 lg:px-[105px] flex items-center justify-between">
            <div>
            ...
            {/* Mobile Burger */}
            <button onClick={toggleMenu} className="md:hidden text-white hover:bg-gray-second/50 p-1 rounded-lg transition-colors duration-300 cursor-pointer">
                {isOpen ? <X size={28} /> : <Menu size={28} />}
            </button>

            {/* Mobile Menu */}
            {isOpen && (
                <div className="absolute top-16 left-0 right-0 bg-main-black flex flex-col items-center gap-8 py-8 md:hidden">
